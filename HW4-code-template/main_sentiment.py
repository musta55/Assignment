...
def main():
    ...
    n_layers = 2 ## choose 1-3 layers (increased for better capacity)

    ## input seq length aligned with data pre-processing
    input_len = 150

    ## word embedding length
    # Use 200-dim embeddings (GloVe) for bonus
    embedding_dim = 200
    ...
    output_size = 1
    num_epoches = 15  ## Increased epochs for higher accuracy
    ## please change the learning rate by youself
    learning_rate = 0.002
    # gradient clipping
    clip = 5
    load_cpt = False #True
    ckp_path = 'cpt/sentiment_model.pt'
    ...
    ##-----------------------------------------------------------------------
    # Use the 200d GloVe file (ensure it exists under Glove-embedding/)
    glove_file = 'Glove-embedding/glove.6B.200d.txt'
    ...
    if mode == 'train':
        ...
        for epoches in range(start_epoch, num_epoches):
            ...
            _save_checkpoint(ckp_path, model, epoches, num_batches, optimizer)
